...
function WorkflowNode({ label, Icon, color, delay }: { label: string; Icon: React.ElementType; color: string; delay: number }) {
  return (
    ...
    >
      ...
      <span className="text-[11px] text-white/55 whitespace-nowrap">{label}</span>
    </motion.div>
  )
}
      ...
      <div className="relative mx-auto max-w-6xl">
        <div className="grid lg:grid-cols-2 gap-12 lg:gap-20 items-center">
          ...
          >
            <div className="rounded-2xl border border-white/[0.07] bg-white/2 p-6 backdrop-blur-sm">
              ...
              <div className="flex items-center justify-between mb-6">
                <div className="flex items-center gap-3">
                  ...
                  <div>
                    <p className="text-sm font-semibold text-white/85">n8n İş Akışı</p>
                    <p className="text-[11px] text-white/35">Aktif · Son tetikleme 2 dk önce</p>
                  </div>
                </div>
              ...
              <div className="space-y-2 mb-5">
                ...
                {/* Node 1 */}
                <div className="flex items-center gap-2 rounded-xl border border-white/[0.07] bg-white/3 px-3 py-2.5">
                  <MessageSquare className="h-3.5 w-3.5 text-cyan-400" />
                  <span className="text-xs text-white/60">AI: Karşılama mesajı oluştur</span>
                ...
                <div className="flex items-center gap-2 rounded-xl border border-white/[0.07] bg-white/3 px-3 py-2.5">
                  ...
                  <span className="text-xs text-white/60">CRM: Müşteri profili oluştur</span>
              ...
              <div className="border-t border-white/6 pt-4">
                <p className="text-[11px] text-white/30 mb-3 uppercase tracking-widest">Entegrasyonlar</p>
            ...
            {/* Floating stat */}
            <motion.div
              className="absolute -bottom-4 -right-4 rounded-xl border border-white/10 bg-black/90 backdrop-blur-sm px-4 py-3"
              animate={{ y: [0, -6, 0] }}
              transition={{ duration: 3.5, repeat: Infinity, ease: 'easeInOut', delay: 0.5 }}
            >
              <p className="text-[10px] text-white/35 mb-0.5">Verimlilik Artışı</p>
              <p className="text-xl font-bold text-green-400">+42%</p>
              <p className="text-[9px] text-white/25">Standart otomasyona göre</p>
            </motion.div>
          </motion.div>
          ...
          <div className="order-1 lg:order-2">
            ...
            >
              <span className="text-white/90">İş Akışlarınıza</span>
              <br />
            ...
            >
              {STATS.map((s, i) => (
                <div key={i} className="rounded-xl border border-white/6 bg-white/2 p-4 text-center">
